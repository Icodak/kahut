...
public interface ConversationRepository extends JpaRepository<Conversation, UUID> {
    ...
    @Query( value = "SELECT c.id FROM conversation c WHERE c.sender_id = :email OR c.recipient_id = :email", nativeQuery = true)
    List<UUID> getByAuthor(@Param("email") String email);
